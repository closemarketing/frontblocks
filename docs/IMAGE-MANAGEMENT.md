...
| Limit max upload dimension | On by default (2048px, overriding WordPress core's own 2560px default): on upload, the stored original is downscaled if it exceeds the configured width/height, so an oversized source image doesn't bloat storage or slow down thumbnail generation. Configurable to a different value (300–10000px), or can be turned off entirely to keep full-size originals untouched. |
...
| WebP quality / AVIF quality | Compression quality (1–100), set independently per format — AVIF defaults lower (60 vs. 82) since the same numeric quality produces a noticeably smaller AVIF file at a visually comparable result. |
