...
class Popups {
	...
	public function save_meta( $post_id ) {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Nonce verified above; each key is sanitized individually below.
		...
		$data = array(
			...
			'bg_color'            => sanitize_hex_color( $raw['bg_color'] ?? '#ffffff' ) ? sanitize_hex_color( $raw['bg_color'] ) : '#ffffff',
			'close_color'         => sanitize_hex_color( $raw['close_color'] ?? '#333333' ) ? sanitize_hex_color( $raw['close_color'] ) : '#333333',
		);
	...
	private function matches_current_page( $meta ) {
		...
		if ( 'specific_pages' === $where ) {
			...
			return in_array( $page_id, array_map( 'intval', (array) $meta['specific_pages'] ), true );
		}
