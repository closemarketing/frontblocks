    ...
    if (carouselItem.length > 0) {
        carouselItem.forEach((item) => {
            ...
            const glideFrontBlocks = new Glide(wrapperParent, {
                ...
                // pauseReasons) so it stays coordinated across both reasons
                // rather than fighting over Glide's internal play/pause state.
                hoverpause: false,
            ...
            if (effectiveAutoplay > 0 && carouselPauseOnHover) {
                // Coordinates every reason autoplay might be paused (hover,
                // keyboard/AT focus) so they can't fight over Glide's
                // play/pause state — autoplay only resumes once neither
                // applies any more.
