		...
		return function(props) {
			...
			useEffect(function() {
				if (!frblCustomSvgAnimationEnabled || !frblCustomSvgAnimationJson) {
					// Clear interval if animation is disabled.
					...
					if (isLottie && typeof lottie !== 'undefined') {
						...
						setTimeout(function() {
							...
							if (lottieContainer) {
								...
								// Apply color from Shape block styles - MEJORADO CON INTERVALO.
								if (svgStyles.fill) {
									...
									// Wait for Lottie to fully render.
									animation.addEventListener('DOMLoaded', function() {
										applyColorToLottie(lottieContainer, svgStyles.fill);
										
										// Start interval to keep applying color (for animations with color keyframes).
										colorIntervalRef.current = setInterval(function() {
											applyColorToLottie(lottieContainer, svgStyles.fill);
										}, 100); // Re-apply every 100ms
									});
									
									// Also apply immediately in case event already fired.
									setTimeout(function() {
										applyColorToLottie(lottieContainer, svgStyles.fill);
									}, 200);
								}
